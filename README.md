...
一个移动优先的中文景点语音导览 MVP。它是浏览器里的 PWA，不是原生 App。第一版使用本地静态数据，包含汉堡、阿尔萨斯、因特拉肯与伯尔尼高地、蒙特勒湖区和巴黎的 34 个导览点；另外加入卢浮宫导览实验模块，用地图、楼层和展品结构组织博物馆内容。
...
- 卢浮宫模块：按 Denon / Sully / Richelieu 三翼、楼层和重点展品建立简化导览地图
- 卢浮宫 Top 100 内容结构：先拆成 10 个章节，再用 Prompt 批量生成作品解说 JSON
- 卢浮宫 Prompt 页面：`/louvre/prompt` 可直接复制给 ChatGPT 生成解说文字
...
- 地图：城市景点页提供 Google Maps 跳转链接；卢浮宫模块使用简化馆内地图，并链接官方 PDF 与互动地图。官方地图和房间开放状态会变化，导入展品数据时应保留 `lastVerified` 和 `locationNote`。
